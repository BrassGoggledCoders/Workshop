...
public class MortarRecipe extends WorkshopRecipe {
    ...
    public FluidTagInput fluidInput = null;
    ...
    public MortarRecipe(ResourceLocation name, Ingredient[] input, FluidTagInput fluidInput, ItemStack output, int cooldownTime) {
        this(name);
        this.input = input;
    ...
    public boolean matches(@Nonnull IItemHandler handler, FluidTankComponent<MortarTileEntity> tank) {
        ...
        return InventoryUtil.inventoryHasIngredients(handler, input) && fluidInput.test(tank.getFluid());
    }
