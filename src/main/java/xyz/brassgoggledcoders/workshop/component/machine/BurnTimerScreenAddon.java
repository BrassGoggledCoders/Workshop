...
public class BurnTimerScreenAddon<T extends IComponentHarness> extends BasicScreenAddon {
    ...
    public void drawBackgroundLayer(Screen screen, IAssetProvider provider, int guiX, int guiY, int mouseX, int mouseY, float partialTicks) {
        ...
        render(screen, guiX, guiY, provider, this);
    }
    ...
    public List<String> getTooltipLines() {
        ...
        tooltip.add(TextFormatting.RED + "Ticks Remaining: " + TextFormatting.WHITE + new DecimalFormat().format(progressBar.getProgress()) + TextFormatting.GOLD + "/" + TextFormatting.WHITE + new DecimalFormat().format(progressBar.getMaxProgress()));
        ...
        tooltip.add(TextFormatting.RED + "ETA: " + TextFormatting.WHITE + new DecimalFormat().format(Math.ceil(progress * progressBar.getTickingTime() / 20D)) + TextFormatting.DARK_AQUA + "s");
        return tooltip;
    }

    public <H extends IComponentHarness> void render(Screen screen, int guiX, int guiY, IAssetProvider provider, BurnTimerScreenAddon<H> addon) {
